...
async function getsongsname(link){
    ...
    for( i of as){
        if(i.href.includes("hollywoodsongs/")){
            songsUl.push(i.href)
        }
    }
    return songsUl
...
async function writesongs(link){
    ...
    for(i of songs){
        let l=(i.split("/songs/hollywoodsongs/"))[1]
    ...
    Array.from(document.querySelector(".songslist").getElementsByTagName("div")).forEach(e=>{
        e.addEventListener("click",()=>{
            ...
            let src="songs/hollywoodsongs/"+song+".mp3"
    ...
    previous.addEventListener("click",()=>{
        ...
        let l=(src.split("/songs/hollywoodsongs/"))[1]
        let a=(l.replaceAll("%20"," "))
        let name=a.split(".")[0]
